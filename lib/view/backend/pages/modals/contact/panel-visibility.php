<div id="tab_panel_visibility" class="panel qltgm_options_panel hidden" style="display: none;">
	<div class="options_group">
		<p class="form-field">
		<label><?php esc_html_e( 'Devices', 'quadlayers-telegram-chat' ); ?></label>
		<select name="display[devices]" data-placeholder="<?php echo esc_html__( 'Choose target&hellip;', 'quadlayers-telegram-chat' ); ?>" aria-label="<?php echo esc_html__( 'Posts', 'quadlayers-telegram-chat' ); ?>" class="qltgm-select2">
	...
	<div class="options_group">
		<p class="form-field">
		<label><?php esc_html_e( 'Target', 'quadlayers-telegram-chat' ); ?></label>
		<select name="display[target][include]" class="qltgm-select2"> 
		...
		<select multiple="multiple" name="display[target][ids][]" data-placeholder="<?php echo esc_html__( 'Choose target&hellip;', 'quadlayers-telegram-chat' ); ?>" aria-label="<?php echo esc_html__( 'Posts', 'quadlayers-telegram-chat' ); ?>" class="qltgm-select2">
		...
		<span class="description hidden"><?php esc_html_e( 'If you select an option all the other will be excluded', 'quadlayers-telegram-chat' ); ?></span>
		</p>
	</div> 
	<?php foreach ( $contact_entries as $key => $entry ) : ?>
		<div class="options_group qltgm-premium-field">
		<p class="form-field">
			<label><?php esc_html_e( ucwords( $entry->label ) ); ?></label>
			<select name="display[entries][<?php echo esc_attr( $key ); ?>][include]" class="qltgm-select2"> 
			...
			<select multiple="multiple" id="qltgm_select2_<?php echo esc_attr( $key ); ?>" data-placeholder="<?php echo esc_html__( 'Search by title&hellip;', 'quadlayers-telegram-chat' ); ?>" name="display[entries][<?php echo esc_attr( $key ); ?>][ids][]" data-name="<?php echo esc_attr( $key ); ?>" class="qltgm-select2-search"> 
	...
	<?php foreach ( $contact_taxonomies as $key => $taxonomy ) : ?>
		...
		<p class="form-field">
			...
			<select name="display[taxonomies][<?php echo esc_attr( $key ); ?>][include]" class="qltgm-select2"> 
			<option <# if ( data.display.taxonomies['<?php echo esc_attr( $key ); ?>']['include'] == 1 ) { #>selected="selected"<# } #> value="1"><?php esc_html_e( 'Include', 'quadlayers-telegram-chat' ); ?></option>
			<option <# if ( data.display.taxonomies['<?php echo esc_attr( $key ); ?>']['include'] == 0 ) { #>selected="selected"<# } #> value="0"><?php esc_html_e( 'Exclude', 'quadlayers-telegram-chat' ); ?></option> 
			</select> 
			<select multiple="multiple" id="qltgm_select2_<?php echo esc_attr( $key ); ?>" data-placeholder="<?php echo esc_html__( 'Choose target&hellip;', 'quadlayers-telegram-chat' ); ?>" name="display[taxonomies][<?php echo esc_attr( $key ); ?>][ids][]" data-name="<?php echo esc_attr( $key ); ?>" class="qltgm-select2"> 
			<?php
			$terms = get_terms(
				array(
					'taxonomy'   => $key,
